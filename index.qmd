...
author: "Consuelo Alzamora" # Remember to add your name!
theme: united
---
...
cantril_table <- cantril %>% 
  ...
  mutate(`Absolute change` = round(`2023` - `2011`, 2), 
         ...
         `Relative change` = percent(`Relative change`))
...
cantril_filtered <- cantril %>%
  filter(Country %in% c("Chile", "Haiti", "United States", "Brazil", "France"))

cantril_plot <- 
  ggplot(cantril_filtered, 
                       aes(x = Year, 
